...
public class CalculatorClient {
    ...
    private static void doMax(final ManagedChannel channel) throws InterruptedException {
        ...
        final StreamObserver<MaxRequest> stream = stub.max(new StreamObserver<MaxResponse>() {
            @Override
            public void onNext(final MaxResponse maxResponse) {
        ...
        Arrays.asList(1, 5, 3, 6, 2, 20)
                .forEach(number -> stream.onNext(MaxRequest.newBuilder().setNumber(number).build()));
