...
public class CalculatorServiceImpl extends CalculatorServiceGrpc.CalculatorServiceImplBase {
    ...
    @Override
    public StreamObserver<AvgRequest> avg(final StreamObserver<AvgResponse> responseObserver) {
         return new StreamObserver<AvgRequest>() {
             int sum = 0;
             int count = 0;

             @Override
             public void onNext(final AvgRequest avgRequest) {
    ...
    public StreamObserver<MaxRequest> max(final StreamObserver<MaxResponse> responseObserver) {
        return new StreamObserver<MaxRequest>() {
            ...
            @Override
            public void onNext(final MaxRequest maxRequest) {
